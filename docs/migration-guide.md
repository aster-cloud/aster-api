...
### 3.2 ⚠️ 中文（zh）当前限制：两个连续 `定义` 暂不支持

实测发现 zh 词法包下，**两个或更多连续的 `定义`(Define) 声明会编译失败**（报 `Expected '.'`）。
单个 `定义`、`定义`+`规则`、`规则`+`定义` 均正常；仅 `定义`+`定义` 触发。en/de 无此问题。

**影响**：需要"输入类型 + 输出结构体"两个 `定义` 的 zh 规则当前编不过。
**临时规避**：zh 规则只声明一个输入 `定义`，规则用 `产出 文本`（`produce Text`）直接返回文本结论，
不构造输出结构体（生产环境的信贷 demo 即用此形态）。
**状态**：已知引擎缺陷，待修复（需双引擎对齐 + 发布级联）。迁移 zh 规则前请确认此限制是否影响你的规则形态。
