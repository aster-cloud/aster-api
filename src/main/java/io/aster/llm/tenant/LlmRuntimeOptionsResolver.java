 ...
 * 有合法 BYOK 覆盖 → BYOK 凭证：apiKey=用户解密 key，provider 经 allowlist 规范化，
 * <b>baseUrl 由本类按 provider 查固定表得到，绝不接受外部传入的 baseUrl（防 SSRF）</b>。
 *
...
public class LlmRuntimeOptionsResolver {
    ...
    public LlmRuntimeOptions resolve(String tenantId, ByokOverride byok) {
        ...
        String baseUrl = BYOK_PROVIDER_BASE_URLS.get(provider);
        if (baseUrl == null) {
            // 红队：不接受未知 provider（防 SSRF / 打错端点），且不回退平台 key。
            throw new IllegalArgumentException("不支持的 BYOK provider: " + provider);
        }
        LOG.infof("使用 BYOK 凭证: tenant=%s, provider=%s", tenantId, provider);
